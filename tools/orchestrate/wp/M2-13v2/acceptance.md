Repo root: /Users/rutmehta/Developer/tessera (cd there first). Perform section A steps 1-4, then section L (Develop panels) only. Every control has an accessibility identifier (see the Identifiers appendix at the end): target controls by identifier/label, use keyboard nudges for exact slider values (arrow keys, ⇧ for x10) instead of dragging, and read values from the readouts. Screenshot at each camera step and on any failure. Report any visual defect with a screenshot. Quit the app at the end.
