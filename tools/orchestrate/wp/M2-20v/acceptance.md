Repo root: /Users/rutmehta/Developer/tessera (cd there first). Perform section A steps 1-3 (skip 4), then section O (Export and Print) only. Use the --export-selftest and --print-pdf-selftest flags where the script offers them; otherwise drive the sheets with keyboard where possible. Screenshot at each camera step and on failure; note any dialog text. Quit the app at the end.
