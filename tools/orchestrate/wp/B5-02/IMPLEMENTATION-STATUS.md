# B5-02 implementation status: document mode UI over a stub backend

Branch `wp/B5-02`. Everything runs over `StubDocumentBackend`; the real `DocumentSession` (B5-01) is wired in B5-03.
...
`DocumentBackend` is B5-01's `DocumentSessionProtocol` (branch `wp/B5-01`, e050cdf / 92bef76) call for call, same labels
and throws. Records carry distinct Swift names so files importing both TesseraCore and TesseraFFI are not ambiguous:

| TesseraCore | TesseraFFI (B5-01) |
| --- | --- |
...
ThemeLintTests stays green. `xcodebuild … -derivedDataPath ~/.cache/tessera-derived-data-B5-02 build`: BUILD SUCCEEDED.
...
## What B5-03 does to wire the real session

1. Merge `wp/B5-01` (bindings regenerated by `build-ffi.sh`).
...
4. Verify against the engine: history node 0 for "as opened" (the Opened row calls `checkoutHistory(id: 0)`),
   ...
   `set_viewport` in level pixels, which matches B5-01's doc), and that `set_selection_rect` / `clear_selection` create
   history nodes (the UI already expects this).
5. Run ACCEPTANCE §U part 2.

## Deviations and notes

- **Names**: TesseraCore records are renamed (table above) instead of reusing B5-01's names; identical names would be
  ambiguous in every file importing both modules (B5-01 flagged this).
- **Protocol grew to B5-01's final API**: besides the brief's calls it has `id`, `layer`, `setSelectedLayers`,
