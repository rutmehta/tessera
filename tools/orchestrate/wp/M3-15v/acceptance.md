Repo root: /Users/rutmehta/Developer/tessera (cd there first). Perform section A steps 1-4, then section S (keywords, captions, OCR; use --fake-captioner as the script says) and section T (tethered capture; use --fake-tether as the script says, and put --tether last in the launch arguments). Target controls by accessibility identifier (appendix). Screenshot at each camera step and on any failure; report visual defects with a screenshot. Quit the app at the end.
