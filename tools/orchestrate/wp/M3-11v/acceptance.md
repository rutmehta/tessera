Repo root: /Users/rutmehta/Developer/tessera (cd there first). Perform section A steps 1-4, then section Q (assisted culling; use --seed-faces and --defects as the script says) and section R (agent review; use --fake-planner). Target controls by accessibility identifier (appendix). Screenshot at each camera step and on any failure; report visual defects with a screenshot. Quit the app at the end.
