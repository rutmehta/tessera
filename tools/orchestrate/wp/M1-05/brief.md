...
4. White balance: multiply by as-shot coefficients normalised to green.
5. Camera colour: camera→XYZ (from raw metadata matrix, inverted as LibRaw provides) → CAT16 adaptation from the shot illuminant to D65 (use engine_api::color) → linear Rec.2020.
