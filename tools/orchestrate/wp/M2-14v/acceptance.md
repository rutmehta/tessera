Repo root: /Users/rutmehta/Developer/tessera (cd there first). Perform section A steps 1-3 (skip 4), then section M (Masks) only. Prefer keyboard shortcuts and the --masks-selftest flag where the script offers them; when a step needs a mouse drag, use computer_use drag with explicit start/end coordinates and take a screenshot before and after. Screenshot at each camera step and on failure. Note any dialog text. Quit the app at the end.
