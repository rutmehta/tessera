Repo root: /Users/rutmehta/Developer/tessera (cd there first). This is a full regression pass on the redesigned app. Perform section A steps 1-4, then sections B (group navigation), K/L (library + develop panels), and M (masks). Every control now has an accessibility identifier listed in the Identifiers appendix at the end of the script: use find/read_page-style element lookup by identifier or label rather than screen coordinates wherever possible. Screenshot at each camera step and on any failure. Also report any visual defect you notice (overlap, misalignment, clipped text, inconsistent control sizes) with a screenshot, even if the step passes. Quit the app at the end.
