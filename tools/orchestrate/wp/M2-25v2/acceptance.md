Repo root: /Users/rutmehta/Developer/tessera (cd there first). Full regression pass on the current app: perform section A steps 1-4, then sections B, K, L, M, and T. Target controls by accessibility identifier (appendix); use keyboard nudges for exact slider values. Screenshot at each camera step and on any failure; report every visual defect (overlap, clipped text, misalignment, wrong control size) with a screenshot even when the step passes. Quit the app at the end.
