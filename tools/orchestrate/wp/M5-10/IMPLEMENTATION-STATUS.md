# M5-10 implementation status: document mode UI over a stub backend

Branch `wp/M5-10`. Everything runs over `StubDocumentBackend`; the real `DocumentSession` (M5-09) is wired in M5-10b.
...
ThemeLintTests stays green. `xcodebuild … -derivedDataPath ~/.cache/tessera-derived-data-M5-10 build`: BUILD SUCCEEDED.
...
## What M5-10b does to wire the real session
