Repo root: /Users/rutmehta/Developer/tessera (cd there first). Perform section A steps 1-3 (build + scratch data; skip 4), then section K (Library) and section L (Develop panels) of the script below; skip everything else. Screenshot with the computer_use screenshot tool at every camera step and on any failure. If the app shows a dialog at launch, note its exact text. Quit the app at the end.
