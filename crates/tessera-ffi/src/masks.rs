...
// One implementation shared with the mask-ai crate consumed by export.
// Keep the FFI dependency surface unchanged.
#[path = "../../mask-ai/src/lib.rs"]
mod mask_backend;
use mask_backend::{AlphaPlane, object_prompts, orient, reorient, resample, unorient};
pub use mask_backend::{MaskSegmenter, SegmentRequest};
...
impl Engine {
    ...
    pub(crate) fn with_segmenter<R>(
        &self,
        f: impl FnOnce(&mut dyn MaskSegmenter) -> anyhow::Result<R>,
    ) -> anyhow::Result<R> {
        let mut slot = self.segmenter.0.lock().unwrap_or_else(|e| e.into_inner());
        if slot.is_none() {
            *slot = Some(self.load_segmenter()?);
        }
        f(slot.as_mut().expect("loaded").as_mut())
    }

    fn load_segmenter(&self) -> anyhow::Result<Box<dyn MaskSegmenter>> {
        use anyhow::Context;
        let support = self.db.parent().context("support directory")?;
        mask_backend::load_segmenter(support)
    }
}
...
impl AiMaskJob {
    ...
    fn compute(&self, shared: &Arc<Shared>, ctx: &JobContext) -> anyhow::Result<AlphaPlane> {
        ...
        ctx.check_cancelled()?;
        let request = mask_backend::request(&self.kind, orientation)?;
