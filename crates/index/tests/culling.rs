...
fn upgrade_v3_preserves_images_and_selection() {
    ...
    conn.execute_batch(
        "DROP TABLE face; DROP TABLE export_log; DROP TABLE score; DROP TABLE accepted_keyword; DROP TABLE understanding; DROP TRIGGER image_fts_delete; DELETE FROM migration WHERE version>=4;",
    )
    .unwrap();
    drop(conn);
    let index = Index::open(&db).unwrap();
