...
- `schema_version`
- `image_id`
- `source_kind: SourceKind` (`raw` or `rgb`; missing = `raw`)
...
3. **The recipe hash covers exactly the render state.** Selection, history, snapshots, id counters, `provenance` and `unknown` never feed `recipe_hash()` or `stage_chain()`. Anything that changes pixels must live in `settings`, `process_version` or the source route `source_kind`. Raw hashes keep their 1.2 encoding; RGB hashes include the source kind and use a distinct stage-chain seed.
...
- 1.3.0 (M5-20), recipe schema 3:
  ...
  - Added `ChannelId` (numeric), `ChannelKind::{Alpha, Spot { display_rgb, solidity }}`, `ChannelSummary`, and ordered `DocumentSummary.channels`, defaulting to empty for 1.2 summaries. Channels are document-owned, not derived from the active selection. Duplicate names are valid; IDs are document-local, monotonic and separate from layers. `SelectionId(n)` refers to alpha `ChannelId(n)` only; the conversion in the other direction requires checking the kind.
  - Added `add_channel`, `delete_channel`, `rename_channel`, `edit_channel`, `load_channel_as_selection` to `DocumentToolCall` and the append-only action registry. All are edits with the existing rationale/group/three-state `expect_head` envelope. Load accepts alpha AND spot channels and changes only the active selection. Add allocates a fresh ID and appends in document order; edits retain order and ID. Hosts reject unknown IDs, invalid finite/range metadata and raster shape mismatches before modifying state.
  - `ChannelRasterRef { digest, depth, extent }` addresses an immutable, host-staged single-channel raster. It provides full-raster replacement without embedding pixel buffers in tool calls. The host resolves the BLAKE3 digest in its raster store, checks canvas-sized extent and finite normalized mask samples, and retains the referenced content for action replay. No URI/network fetch is implied. This is a contract only; raster staging and channel command execution are follow-up integration work. Alpha/spot kind uses a `kind` tag. `display_rgb` is finite [0,1] RGB (compositor adapters map its existing `color` field explicitly); solidity is finite [0,1]. Spot metadata does not yet implement spectral preview or affect RGB compositing.
  - Added `people` contracts: existing numeric `PersonId` is unchanged; `FaceRef { image_id, ordinal }` is composite and invalidated on re-detection. `PersonSummary { id, name, confirmed_count, approximate }` carries stable catalog identities, including unnamed or empty identities. No queue-local `person-N` identity is introduced. Adapters to crate-local string IDs must preserve catalog-stable mappings, not parse queue labels.
  - `FaceRegion` uses normalized centre/size plus oriented analysis-preview pixel dimensions, not `NormalizedRect` edge coordinates. Hosts require finite geometry and positive coordinate-space dimensions. `NameSuggestion` carries a stable person ID, optional name, and cosine similarity in [-1,1], NOT a probability; suggestions never mutate assignments.
  - `ClusterOptions` carries cosine threshold (default 0.363), `QualityGate` (confidence >=0.9, both preview dimensions >=32, sharpness >=0.1) and positive refit interval (default 1000 newly observed faces). Rejected/descriptor-less faces remain manually assignable but do not train medoids. `PeopleJobResult { assigned, refit, approximate }` reports a host-driven invocation, not a scheduler or an exact full-library hierarchy. Named identities and confirmed assignments are protected by automatic jobs.
  - Added separate `LibraryToolCall` and `LibraryToolRequest`: `assign_person`, `confirm_person` (explicit confirm/unconfirm), `merge_people` (source into surviving target), `split_person` (fresh unnamed identity, moved confirmations reset), `name_person` (required nullable name). All carry `PeopleWriteOptions` with independent `write_sidecars` and `person_keywords` opt-ins, both false by default. A keyword opt-in alone does not authorize sidecar I/O; sidecar keywords require both flags. Keywords are additive and renaming does not remove old ones. Hosts serialize and validate catalog operations. Catalog/XMP writes are not crash-atomic together. Library actions use `CommandDomain::Library`, `CommandEffect::Effect`, and rationale for audit, not recipe/document history groups.
  - `Recipe.source_kind` consumes the former top-level `source_kind` string extension directly rather than leaving it in `unknown`. This is a wire-compatible alias of the old extension spelling, not a nested `unknown` object. Canonical strings are `raw` and `rgb`; `Raw`/`Rgb` are also accepted and normalized. Absence always infers `Raw`, never guesses from filename, settings or provenance. Importers which know the source is RGB must explicitly set it. Invalid/null values are rejected, unrelated extensions survive, and `from_json` upgrades schema 1/2 to 3. Saving normalized loaded recipes writes one typed field. New code must not insert reserved typed names into `Recipe.unknown`.
  - Source route is render-affecting: RGB recipe hashes include `source_kind`, and stage chains use a separate source/process seed. Raw omits the extra hash member and keeps the old process seed, preserving the 1.2 golden raw hash. Native process revision is unchanged because no rendering operator changed. Route is source identity, not a develop-history patch.
  - JSON 1.2 compatibility fixtures and 1.3 typed schema/round-trip fixtures cover the new types and all new calls. This is serialized backward compatibility, NOT Rust source compatibility: exhaustive document/action matches and full `DocumentSummary`/`Recipe` literals need caller changes. `DocumentSummary` no longer implements `Copy` or `Eq`. No callers or host bindings are wired in this package.
