...
3. **No "bit-for-bit" GPU/CPU claim.** wgpu's Metal path compiles with fast-math and WGSL permits FMA/reassociation, so spec 07 §1's "bit-identical on any GPU/CPU" is replaced by: deterministic per backend, and GPU output gated against the CPU reference at a tolerance: per operator max abs error ≤ 1e-4 linear; for a full multi-stage chain ≤ 2e-3 linear and ≤ 1 code value in 8-bit display output (M2-05 measured 8.6e-4 on the Nikon NEF full chain, 2026-09-25); ΔE2000 ≤ 0.5 on the golden suite.
...
### 2.1 Two-machine operation and supervisor fallback (2026-09-26)

- Two laptops share the account quotas (Codex resets daily, Anthropic usage is pooled), so they double throughput, not budget. Split by area, not by file: the main session (coordinator) owns engine crates (Astra-heavy) and all merges to `main`; the second machine's session owns the M5 layered-editor UI packages (Opus-heavy, Xcode) — M5-09 FFI `DocumentSession`, M5-10 document mode UI, M5-11 tools, M5-12 filters — and pushes `wp/*` branches only, then reports the ids for merge. Neither machine edits `Cargo.lock` or `tools/orchestrate/board.json` outside its own `wp/*` branch.
- **Supervisor fallback**: when the Fable usage limit runs out, GPT-6 Astra (900k context) through the Codex CLI (`codex` with the ChatGPT login) takes over as the supervisor/planner: it reads this plan, `docs/STATUS.md` and the board, writes briefs in the same format, launches Opus/Astra/Luna runs through `tools/orchestrate/`, and merges on green. Briefs therefore always carry absolute test commands and allowed paths so any supervisor can run them unchanged.
